...
void UASView::updateMode(int sysId, QString status, QString description)
{
    ...
    //QString modStatus = status.remove(0,2);
    QString modStatus = this->uas->getShortMode();
    QString colorstyle("QLabel { background-color: %1; }");

    if(modStatus == "D|AUTO" || modStatus == "A|AUTO")
    {
        flightTypeColor = Qt::green;
        m_ui->flightTypeLabel->setStyleSheet(colorstyle.arg(flightTypeColor.name()));
        m_ui->flightTypeLabelLegend->setText(modStatus);
    }
    else if(modStatus == "D|MANUAL" || modStatus == "A|MANUAL")
    {
        flightTypeColor = Qt::red;
        m_ui->flightTypeLabel->setStyleSheet(colorstyle.arg(flightTypeColor.name()));
        m_ui->flightTypeLabelLegend->setText(modStatus);
    }
    else if (modStatus == "D|UNKNOWN" || modStatus == "A|UNKNOWN")
    {
        flightTypeColor = Qt::yellow;
        m_ui->flightTypeLabel->setStyleSheet(colorstyle.arg(flightTypeColor.name()));
        m_ui->flightTypeLabelLegend->setText(modStatus);
    }
    else
    {
        flightTypeColor = Qt::cyan;
        m_ui->flightTypeLabel->setStyleSheet(colorstyle.arg(flightTypeColor.name()));
        m_ui->flightTypeLabelLegend->setText(modStatus);
    }
